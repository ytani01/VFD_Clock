...
#define BUTTON_DEBOUNCE   200  // msec
#define BUTTON_LONGPRESSED_COUNT    200
...
void loop() {
  ...
  if ( ButtonMode_PressCount > 0 ) {
    if ( digitalRead(PIN_BUTTON_MODE) == LOW ) {
      ...
      if ( ! ButtonMode_LongPressed ) {
        if (ButtonMode_PressCount % 100 == 0 ) {
  ...
  if ( ButtonCount_PressCount > 0 ) {
    if ( digitalRead(PIN_BUTTON_COUNT) == LOW ) {
      ...
      if ( ! ButtonCount_LongPressed ) {
        if ( ButtonCount_PressCount % 100 == 0 ) {
          Serial.print("ButtonCount_PressCount: ");
          Serial.println(ButtonCount_PressCount);
        }
        if ( ButtonCount_PressCount > BUTTON_LONGPRESSED_COUNT * DigitN / 2 ) {
  ...
  if ( ButtonCount_PressCount == 2 || ( ButtonCount_LongPressed && (ButtonCount_PressCount % 200 == 0)) ) {
  ...
  dp = false;
  if ( (DigitI == 1 || DigitI == 3) && (dispBuffer[DigitI] != NUM_CLR) ) {
    dp = true;
  }
  dispOneDigit(DigitN - DigitI - 1, dispBuffer[DigitI], dp);
  delay(DISP_DELAY);
} // loop()
